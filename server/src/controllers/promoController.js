...
async function list(req, res) {
  ...
  const where  = ['1=1'];
  const params = [];
  if (status === 'active')   { where.push('is_active = 1 AND (expires_at IS NULL OR expires_at > NOW())'); }
  if (status === 'inactive') { where.push('is_active = 0'); }
  if (status === 'expired')  { where.push('expires_at IS NOT NULL AND expires_at <= NOW()'); }

  const [rows] = await pool.query(
    `SELECT * FROM promo_codes WHERE ${where.join(' AND ')} ORDER BY created_at DESC`, params
  );
  res.json(rows);
}
