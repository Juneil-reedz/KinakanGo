...
export default function RiderLogin() {
  ...
  const [form, setForm]         = useState({ email:'rider@fooddelivery.com', password:'demo123' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) { showSuccess('Welcome back, Rider!'); setTimeout(() => navigate('/rider/dashboard'), 800); }
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-up">
        ...
        <div className="glass rounded-3xl p-8 card-3d" style={{ boxShadow:'0 32px 64px rgba(0,0,0,.5)' }}>
          <div className="flex flex-col items-center mb-7">
            <div className="w-16 h-16 btn-glow-orange rounded-2xl flex items-center justify-center text-3xl mb-3 animate-breathe">🛵</div>
            <h1 className="text-2xl font-heading font-bold text-white">Rider Portal</h1>
            <p className="text-forest-200/60 text-sm mt-1">Sign in to start delivering</p>
          </div>

          <div className="glass-green rounded-xl px-4 py-3 mb-5">
            <p className="text-forest-200 text-xs"><span className="font-semibold text-white">Demo:</span> rider@fooddelivery.com / demo123</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={form.email} onChange={set('email')} placeholder="rider@example.com"
          ...
          <p className="text-center text-forest-200/50 text-sm mt-5">
            ...
            <Link to="/rider-application" className="text-ember-400 font-semibold hover:text-ember-300 transition-colors">Apply here</Link>
          </p>
        </div>
      </div>
    </div>
