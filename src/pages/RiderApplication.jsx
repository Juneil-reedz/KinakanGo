...
const STEP_LABELS = ['Personal', 'Emergency', 'Vehicle', 'Documents'];

const EMPTY = {
  fullName: '', dateOfBirth: '', address: '', contactNumber: '', email: '',
  emergencyContact: '', emergencyName: '',
  vehicleType: '', vehiclePlateNumber: '', vehicleModel: '',
  driverLicense: null, vehicleRegistration: null, nbiClearance: null,
  validId: null, proofOfAddress: null, profilePhoto: null, idType: '',
};

function FileUpload({ label, hint, file, onChange, inputId }) {
  return (
    <div>
      <label className="block text-forest-200/60 text-xs font-medium mb-1.5">
        {label} <span className="text-red-400">*</span>
      </label>
      {hint && <p className="text-forest-200/40 text-xs mb-1.5">{hint}</p>}
      <div className="border-2 border-dashed border-forest-600/40 rounded-xl p-5 text-center hover:border-ember-500/60 hover:glass-orange transition-all">
        ...
        <label htmlFor={inputId} className="cursor-pointer flex flex-col items-center gap-2">
          <Upload className={`w-6 h-6 ${file ? 'text-forest-400' : 'text-forest-300/30'}`} />
...
export default function RiderApplication() {
  ...
  const setFile = (e, field) => setFormData(p => ({ ...p, [field]: e.target.files[0] }));
  ...
  const validate = () => {
    const errs = {
      ...
      4: () => !formData.driverLicense || !formData.vehicleRegistration || !formData.nbiClearance || !formData.validId || !formData.proofOfAddress || !formData.profilePhoto,
    };
    if (errs[step]?.()) {
      showError(step === 4 ? 'Please complete all required uploads' : 'Please fill in all required fields');
      return false;
    }
    return true;
  };

  const handleNext   = () => { if (validate() && step < 4) setStep(s => s + 1); };
  ...
  return (
    <div className="container-custom max-w-3xl py-10 animate-fade-up">
      ...
      <div className="glass rounded-3xl p-6 md:p-8">
        ...
        {step === 4 && (
          <div className="space-y-5">
            <h2 className="text-white font-heading font-bold text-xl mb-5">Documents &amp; Profile Photo</h2>
            <div className="glass rounded-xl p-4 flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-ember-400 flex-shrink-0 mt-0.5" />
              <p className="text-forest-200/70 text-sm">All documents must be valid and clear. Accepted: PDF, JPG, PNG (max 5 MB each)</p>
            </div>
            <FileUpload label="Profile Photo" hint="Clear photo of yourself for your rider profile" file={formData.profilePhoto} onChange={e => setFile(e, 'profilePhoto')} inputId="profile-photo-upload" />
            <FileUpload label="Driver's License" file={formData.driverLicense} onChange={e => setFile(e, 'driverLicense')} inputId="license-upload" />
            <FileUpload label="Vehicle Registration (OR/CR)" file={formData.vehicleRegistration} onChange={e => setFile(e, 'vehicleRegistration')} inputId="registration-upload" />
            <FileUpload label="NBI or Police Clearance" file={formData.nbiClearance} onChange={e => setFile(e, 'nbiClearance')} inputId="clearance-upload" />
            <div>
              ...
              <FileUpload label="Valid ID Upload" file={formData.validId} onChange={e => setFile(e, 'validId')} inputId="valid-id-upload" />
            </div>
            <FileUpload label="Proof of Address" hint="Utility bill, bank statement, or barangay certificate" file={formData.proofOfAddress} onChange={e => setFile(e, 'proofOfAddress')} inputId="proof-address-upload" />
        ...
        <div className="flex items-center justify-between mt-8 pt-6" style={{ borderTop: '1px solid rgba(255,255,255,.08)' }}>
          ...
          <span className="text-forest-200/40 text-xs">Step {step} of 4</span>
          {step < 4 ? (
